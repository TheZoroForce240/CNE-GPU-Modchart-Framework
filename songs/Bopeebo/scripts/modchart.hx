...
function setupModifiers()
{
	...
	createModifier("angleX", 45, "
		angleX += angleX_value;
	");

	createModifier("angleY", 45, "
		angleY += angleY_value;
	");

	createModifier("angleZ", 45, "
		angleZ += angleZ_value;
	");
	...
	createModifier("tipsySpeed", 2.0, "", -1, -1, 0.0, false);
	createModifier("tipsy", 0.5, "
...
function setupEvents()
{
	...
	for (i in 0...2)
	{
		ease(16 + (i*4), 2, 'cubeInOut', "
			1, reverse,
		");

		ease(18 + (i*4), 2, 'cubeInOut', "
			0, reverse,
		");
	}
